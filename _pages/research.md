---
title: Our Research
layout: splash_v2
permalink: /research/
classes:
  - csdc-page
excerpt: "Research priorities and topic areas at the McGill CSDC."
show_taxonomy_posts: false
---

<div class="csdc-page">
  <section class="csdc-hero">
    <div class="csdc-container">
      <p class="csdc-kicker"><span class="csdc-red">McGill Node</span></p>
      <h1 class="csdc-hero-title">Our Research</h1>
      <p class="csdc-lead">Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.</p>
    </div>
  </section>

  <section class="csdc-section">
    <div class="csdc-container">
      <h2>Three CSDC Research Axes</h2>
      <div class="csdc-pillars csdc-pillars-two">
        <article class="csdc-card" style="border-top:4px solid #c8102e;">
          <h3 class="csdc-card-title">Learning Democratic Citizenship in an Unequal World</h3>
          <p>This axis examines how democratic values, identities, and civic competencies are developed in contexts shaped by social and economic inequality.</p>
        </article>
        <article class="csdc-card" style="border-top:4px solid #c8102e;">
          <h3 class="csdc-card-title">The Practice of Democratic Citizenship</h3>
          <p>This axis focuses on how citizens participate in democratic life through formal and informal practices, including voting, engagement, and collective action.</p>
        </article>
        <article class="csdc-card" style="border-top:4px solid #c8102e;">
          <h3 class="csdc-card-title">Citizen Representation and Governance</h3>
          <p>This axis analyzes how institutions represent citizens and govern democratically, including responsiveness, accountability, and trust in public decision-making.</p>
        </article>
      </div>
    </div>
  </section>

  <section class="csdc-section">
    <div class="csdc-container">
      <h2>Topics Studied in the Lab</h2>
      <div class="csdc-pillars">
        <article class="csdc-card">
          <h3 class="csdc-card-title">Topic Placeholder 1</h3>
          <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
        </article>
        <article class="csdc-card">
          <h3 class="csdc-card-title">Topic Placeholder 2</h3>
          <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
        </article>
        <article class="csdc-card">
          <h3 class="csdc-card-title">Topic Placeholder 3</h3>
          <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
        </article>
        <article class="csdc-card">
          <h3 class="csdc-card-title">Topic Placeholder 4</h3>
          <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.</p>
        </article>
      </div>
    </div>
  </section>
</div>
